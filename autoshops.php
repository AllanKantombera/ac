...
<div style="margin: 4%;">
    <h2>AUTOPARTS SHOPS</h2>

            <?php

        $query = "SELECT * FROM autoparts_shops";
        $result = mysqli_query($conn, $query);

        if ($result) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<div class="grid-container2"><div class="searchcard1">';
                echo '<img src="images/'. $row["logo"] . '" width="120px" style="border-style: solid; border-width: 1px; max-width: 100%;"></div>';
                echo '<div class="searchcard2"><b>' ;
                echo  '<a href="' . $row['page_url'] . '">' . $row['name']. '</a></b><br>';
                echo $row['description'] . "<br><br>";
                echo $row['contacts'] . "<br>";
                echo "<b>Loc: </b>" . $row['location'] . '<br>' ;
                echo  "</div></div>";
            }
        } else {
